...
@Injectable()
export class MessengerSender extends BaseSender {
  private readonly logger = new Logger(MessengerSender.name);
  ...
  async send(
    ...
    try {
      ...
      if (!response.ok) {
        ...
        this.logger.error(
          `Messenger API error (${response.status}): ${errorBody}`,
        );
      ...
      this.logger.error(`Messenger send failed: ${message}`);
