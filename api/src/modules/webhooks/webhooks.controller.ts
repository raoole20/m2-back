...
export class WebhooksController {
  ...
  async handleNewChat(
    ...
    return await this.webhooksService.handleNewChat(body);
  }
  ...
  async handleInbound(
    ...
    @Param('channelId') channelId: string,
    @Req() request: Request,
  ): Promise<{ status: string; messageTypes: MessageCategory[] }> {
    const channelType = this.parseChannelType(channelTypeParam);
    ...
    const messageTypes = result.messages.map((m) =>
      discriminateMessageType(m.contentType),
    );
